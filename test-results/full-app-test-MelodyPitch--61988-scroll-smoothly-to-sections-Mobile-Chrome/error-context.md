...
Error: locator.isVisible: Error: strict mode violation: getByRole('link', { name: /How it works/i }) resolved to 2 elements:
    1) <a href="#how-it-works" class="block px-4 py-2 text-fg-2 hover:text-fg-1 hover:bg-bg-surface-1 rounded-lg transition-colors duration-200 text-sm font-medium">How it Works</a> aka getByRole('link', { name: 'How it Works', exact: true })
